...
self.addEventListener('message', function (event) {
	if (event.data && event.data.framework && event.data.modules) {
		...
		caches.open('fusion_modules').then(function (cache) {
		...
		caches.open('fusion_framework').then(function (cache) {
			return cache.keys().then(function (keys) {
				keys.forEach(function (request) {
...
self.addEventListener('fetch', function (event) {
	if (data && event.request.method === 'GET') {
		...
		if (type) {
			event.respondWith(caches.open('fusion_' + type).then(function (cache) {
